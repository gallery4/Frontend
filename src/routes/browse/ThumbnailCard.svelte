<script lang="ts">
	...
	function getImageSource(name: string, type: string): string {
		switch (type) {
			case 'file':
				...
				switch (filetype)
				{
					case "image": return `/browse/thumb?path=${name}`;
					case "audio": return AUDIO_IMAGE_SOURCE;
					case "video" : return MEDIA_IMAGE_SOURCE
				}
		...
		switch (type) {
			case 'file':
				const filetype = determinFileType(name);
					switch (filetype)
					{
						case "image": return IMAGE_ICON_COLOR;
						case "audio": return AUDIO_ICON_COLOR;
						case "video" : return MEDIA_ICON_COLOR
					}
	...
	const MEDIA_ICON_COLOR = 'teal';
	const IMAGE_ICON_COLOR = 'lightblue';
	const AUDIO_ICON_COLOR = 'yellow';

	const DIRECTORY_IMAGE_SOURCE = `
		data:image/svg+xml;utf8,
		<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16" fill="${DIRECTORY_ICON_COLOR}" class="bi bi-folder " viewBox="0 0 16 16">
			<path d="M.54 3.87.5 3a2 2 0 0 1 2-2h3.672a2 2 0 0 1 1.414.586l.828.828A2 2 0 0 0 9.828 3h3.982a2 2 0 0 1 1.992 2.181l-.637 7A2 2 0 0 1 13.174 14H2.826a2 2 0 0 1-1.991-1.819l-.637-7a2 2 0 0 1 .342-1.31zM2.19 4a1 1 0 0 0-.996 1.09l.637 7a1 1 0 0 0 .995.91h10.348a1 1 0 0 0 .995-.91l.637-7A1 1 0 0 0 13.81 4zm4.69-1.707A1 1 0 0 0 6.172 2H2.5a1 1 0 0 0-1 .981l.006.139q.323-.119.684-.12h5.396z"/>
		</svg>`;

	const ZIP_IMAGE_SOURCE = `
		data:image/svg+xml;utf8,
		<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16" fill="${ZIP_ICON_COLOR}" class="bi bi-file-zip" viewBox="0 0 16 16">
	...
	const MEDIA_IMAGE_SOURCE = `
		data:image/svg+xml;utf8,
		<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16" fill="${MEDIA_ICON_COLOR}" class="bi bi-file-play" viewBox="0 0 16 16">
			<path d="M6 10.117V5.883a.5.5 0 0 1 .757-.429l3.528 2.117a.5.5 0 0 1 0 .858l-3.528 2.117a.5.5 0 0 1-.757-.43z"/>
			<path d="M4 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm0 1h8a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1"/>
		</svg>`;

	const AUDIO_IMAGE_SOURCE = `
		<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16" fill="${AUDIO_ICON_COLOR}" class="bi bi-file-music" viewBox="0 0 16 16">
...
<Card data-id={createElementId(getFilenameFromKey(name, type))}>
	...
	<CardBody style="height: 6em; overflow:hidden;">
