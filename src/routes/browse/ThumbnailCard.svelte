<script lang="ts">
		...
		switch (type) {
			case 'file':
				...
				switch (filetype)
				{
					...
					case "video" : return VIDEO_IMAGE_SOURCE
				}
			...
			case 'file':
				const filetype = determinFileType(name);
					switch (filetype)
					{
						...
						case "video" : return VIDEO_ICON_COLOR
					}
	...
	const VIDEO_IMAGE_SOURCE = `
		data:image/svg+xml;utf8,
		<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16" fill="${VIDEO_ICON_COLOR}" class="bi bi-file-play" viewBox="0 0 16 16">
